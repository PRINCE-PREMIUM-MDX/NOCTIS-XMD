...
const REQUIRED_GROUP = '@PRINCE R TECH';
const REQUIRED_CHANNELS = [
  '@VIOLET CRASHER TEP 1',//channel 1
  '@VIOLET CRASH BACKUP',
    '@VIOLET CRASH BACKUP 3',// backup channel
    '@VIOLET CRASHEACKUP 2'
];

// Social media links
const SOCIAL_LINKS = {
  whatsapp: 'https://whatsapp.com/channel/0029VbDClYr1noz35rhw1X2T',// whatsapp channel
};
...
Welcome, operator — you are now synced with NOCTIS crasher xmd Protocols ⚡

🧾 PAIRING MATRIX:
➤ /pair <wa_number>  
   🔗 Fuse a WhatsApp device into the Noctis Grid  

➤ /unpair <wa_number>  
   ❌ Remove a bonded device from the registry  

➤ /listpairs  
   📂 Display all active bonded sessions
  
➤ /report 
    🗣️ Tell us your complain

➤ /autoload 
    Autoload your sessions 

➤ /runtime
    To check bot uptime hours 
    
───────────────────────────────
✨ Powered by Prince premium dev 🥶 
⚔️ Rule sharp. Move bold. Stay legendary.︎`;
...
bot.onText(/^\/pair\s*$/, requireMembership((msg) => {
  ...
  bot.sendMessage(
    chatId,
    '⚠️ Input Needed Please enter a valid number to start the Queen Leesha link process.💡 Format: /pair 509xxxxxxxxxx',
     { 
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: [
...
bot.onText(/\/pair (.+)/, requireMembership(async (msg, match) => {
  ...
  try {
    if (!text || /[a-z]/i.test(text)) {
      return bot.sendMessage(chatId, '⚠️ Input Needed Please enter a valid number to start the Noctis crasher xmd link process.💡 Format: /pair 234xxxxxxxxxx',
      { 
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: [
    ...
    if (!/^\d{7,15}(\|\d{1,10})?$/.test(text)) {
      return bot.sendMessage(chatId, 'Invalid format /pair 509xxx',
            { 
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: [
    ...
    if (pairedCount >= 30) {
      return bot.sendMessage(chatId, "Pairing limit reached. Try again later or inform my owner t.me/KallmeTrust .",
            { 
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: [
...
bot.onText(/^\/delpair\s*$/, requireMembership((msg) => {
  const chatId = msg.chat.id;
  bot.sendMessage(chatId, 'To proceed enter a phone number in the format: /delpair 509xxxxxxxx',
        { 
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: [
...
💫 Trust System Activated...
Welcome, operator — you are now synced with Queen Leesha Protocols ⚡

🧾 PAIRING MATRIX:
➤ /pair <wa_number>  
   🔗 Fuse a WhatsApp device into the Leesha Grid  
...
───────────────────────────────
✨ Powered by Prince Premium dev ⭐
⚔️ Rule sharp. Move bold. Stay legendary.`;
  ...
  const keyboard = {
    ...
    const caption = `┌─〔 *Bot Command List* 〕  
